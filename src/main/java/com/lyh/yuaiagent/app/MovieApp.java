...
public class MovieApp {
    ...
    public MovieApp(ChatModel openAiChatModel) {
    ...
    public record MovieList(List<MovieVO> reCommandList) {
    }
    ...
    @Resource
    private VectorStore movieAppVectorStore;

    public MovieList recommendMoviesWithRag(String chatId, List<Movieorder> movieorderList) {
        // 使用更简洁的提示构建方式
        StringBuilder prompt = new StringBuilder();
        ...
        if (movieorderList != null && !movieorderList.isEmpty()) {
            for (Movieorder order : movieorderList) {
                prompt.append(order.getMovieid()).append(" ");
                prompt.append(order.getMoviename()).append(" ");
            }
        } else {
            prompt.append("无");
        }
        ...
        MovieList list = chatClient
                ...
                .user(prompt.toString())
                .advisors(spec -> spec.param(CHAT_MEMORY_CONVERSATION_ID_KEY, chatId))
                .advisors(new QuestionAnswerAdvisor(movieAppVectorStore))
                .call()
                .entity(MovieList.class);
        return list;
    }
